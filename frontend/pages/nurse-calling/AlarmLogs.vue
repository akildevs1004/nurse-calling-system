        ...
        <v-card elevation="0" class="mt-2">
          ...
          >
            ...
            <template v-slot:item.battery_level="{ item }">
              ...
              <div v-else :style="'color:black'">{{ item.battery }}%</div>
            </template>
            <template v-slot:item.alarm_status="{ item }">
              <v-icon :style="getPriorityColor(item.alarm_status)"
                >mdi mdi-bell
              </v-icon>
            </template>
          </v-data-table>
  ...
  data: () => ({
    ...
    headers_table: [
      ...
      },
      {
        ...
        align: "center",
        sortable: false,
        key: "battery_level", //sorting
        value: "battery_level", //edit purpose
