      ...
      <div v-if="can(`employee_view`)">
        <v-card elevation="0">
          ...
          >
            ...
            <template v-slot:item.normal_icon="{ item }">
              <img :src="getIcon('normal', item.id)" style="width: 50px" />
            </template>
            <template v-slot:item.warning_icon="{ item }">
              <img :src="getIcon('warning', item.id)" style="width: 50px" />
            </template>
...
export default {
  ...
  methods: {
    noimage(obj) {
      obj.target.src = process.env.BACKEND_URL2 + "no-photo.jpg";
      //return process.env.BACKEND_URL2 + "no-photo.jpg";
    },
    getIcon(file, id) {
      return (
        process.env.BACKEND_URL2 + "monitor_icons/" + id + "_" + file + ".png"
      );
    },
    encrypt() {
    ...
    submit() {
      ...
      if (this.payload.file_normal)
        payload.append(`file_normal`, this.payload.file_normal || {});
      if (this.payload.file_warning)
        payload.append(`file_warning`, this.payload.file_warning || {});

      this.$axios
        .post("devices-category-create", payload, options)
        .then(({ data }) => {
          // this.encrypt(data.record.id);
          this.errors = [];
          this.snackbar = true;
    ...
    update_data() {
      ...
      payload.append(`company_id`, this.$auth.user.company_id);
      payload.append(`name`, this.payload.name);
      payload.append(`id`, this.payload.id);
      if (this.payload.file_normal)
        payload.append(`file_normal`, this.payload.file_normal || {});
      if (this.payload.file_warning)
        payload.append(`file_warning`, this.payload.file_warning || {});
      ...
      this.$axios
        .post("devices-category", payload, options)
        .then(({ data }) => {
          this.errors = [];
          this.snackbar = true;
